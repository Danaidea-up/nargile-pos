...
create table if not exists employees (id bigint generated always as identity primary key, branch_id bigint, name text, email text, role text, created_at timestamp default now());
...
-- V8 update: if employees table already exists without email column:
